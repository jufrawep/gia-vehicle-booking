...
export const Dashboard = () => {
  ...
  const fetchBookings = async (silent = false) => {
    try {
      ...
      logger.info(CTX, "Bookings loaded", {
        count: res.data.data.bookings.length,
      });
    } catch (error: any) {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      logger.info(CTX, "Payments loaded", {
        count: res.data.data.payments?.length || 0,
      });
    } catch (error: any) {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-semibold ${badge.color}`}
      >
        <Icon />
            ...
            <div className="flex">
              {(
                [
                  {
                    key: "bookings",
                    label: `${fr ? "Réservations" : "Bookings"} (${bookings.length})`,
                    icon: <FaCalendar />,
                  },
                  {
                    key: "payments",
                      ...
                      {bookings.map((booking) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td
                              className={`${td} font-bold text-sm text-primary-dark`}
                            >
                              {booking.totalPrice

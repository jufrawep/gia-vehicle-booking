...
const VehicleModal = ({ vehicle, onClose, onSaved }: VehicleModalProps) => {
  ...
  const [form, setForm] = useState<VehicleFormData>(() =>
    vehicle
      ? {
          ...
          location:
            typeof vehicle.location === "object" && vehicle.location !== null
  ...
  const handleSubmit = async () => {
    if (!form.brand || !form.model || !form.pricePerDay) {
      toast.error(
        fr ? "Remplissez les champs obligatoires" : "Fill in required fields",
      );
      return;
    }
    setSaving(true);
    try {
      ...
      toast.error(
        err.response?.data?.message ||
          (fr ? "Erreur de sauvegarde" : "Save error"),
      );
  ...
  const inp =
    "w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary-light focus:border-transparent";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={lbl}>{fr ? "Marque *" : "Brand *"}</label>
              <input
                className={inp}
                value={form.brand}
            ...
            <div>
              <label className={lbl}>{fr ? "Modèle *" : "Model *"}</label>
              <input
                className={inp}
                value={form.model}
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className={lbl}>{fr ? "Année" : "Year"}</label>
              <input
                type="number"
                className={inp}
                value={form.year}
            ...
            <div>
              <label className={lbl}>{fr ? "Immatriculation" : "Plate"}</label>
              <input
                className={inp}
            ...
            <div>
              <label className={lbl}>{fr ? "Places" : "Seats"}</label>
              <input
                type="number"
                className={inp}
                value={form.seats}
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className={lbl}>{fr ? "Catégorie" : "Category"}</label>
              <select
                className={inp}
                value={form.category}
                onChange={(e) =>
                  set("category", e.target.value as VehicleCategory)
                }
              >
                {(
                  [
            ...
            <div>
              <label className={lbl}>{fr ? "Boîte" : "Gearbox"}</label>
              <select
                className={inp}
                value={form.transmission}
                onChange={(e) =>
                  set("transmission", e.target.value as TransmissionType)
                }
              >
                <option value="MANUAL">{fr ? "Manuelle" : "Manual"}</option>
                <option value="AUTOMATIC">
                  {fr ? "Automatique" : "Automatic"}
                </option>
              </select>
            </div>
            <div>
              <label className={lbl}>{fr ? "Carburant" : "Fuel"}</label>
              <select
                className={inp}
                value={form.fuelType}
                onChange={(e) =>
                  set("fuelType", e.target.value as FuelTypeEnum)
                }
              >
                {(
                  ["PETROL", "DIESEL", "ELECTRIC", "HYBRID"] as FuelTypeEnum[]
                ).map((f) => (
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className={lbl}>
                {fr ? "Prix/Jour (FCFA) *" : "Price/Day (FCFA) *"}
              </label>
              <input
                type="number"
                className={inp}
            ...
            <div>
              <label className={lbl}>{fr ? "Statut" : "Status"}</label>
              <select
                className={inp}
                value={form.status}
            ...
            <div>
              <label className={lbl}>{fr ? "Kilométrage" : "Mileage"}</label>
              <input
                type="number"
                className={inp}
                value={form.mileage}
          ...
          <div>
            <label className={lbl}>{fr ? "URL de l'image" : "Image URL"}</label>
            <input
              className={inp}
              value={form.imageUrl}
          ...
          <div>
            <label className={lbl}>{fr ? "Localisation" : "Location"}</label>
            <input
              className={inp}
              value={form.location}
          ...
          <div>
            <label className={lbl}>
              {fr ? "Équipements (virgules)" : "Features (comma-separated)"}
            </label>
            <input
              className={inp}
              value={form.features}
          ...
          <div>
            <label className={lbl}>{fr ? "Description" : "Description"}</label>
            <textarea
              ...
              placeholder={
                fr ? "Description du véhicule..." : "Vehicle description..."
              }
            />
          </div>
        </div>
          ...
          {(vehicle.features ?? []).length > 0 && (
            <div className="flex flex-wrap gap-2">
              {(vehicle.features ?? []).map((f, i) => (
                <span
                  key={i}
                  className="bg-primary-light/10 text-primary-dark text-xs px-2 py-1 rounded-full"
                >
                  {f}
                </span>
              ))}
...
export const AdminDashboard = () => {
  ...
  const [pmtFilters, setPmtFilters] = useState({
    dateFrom: "",
    dateTo: "",
    status: "",
    paymentMethod: "",
  ...
  const fetchData = async (silent = false) => {
    try {
      ...
      const [statsRes, bookingsRes, vehiclesRes, usersRes, paymentsRes] =
        await Promise.all([
          bookingAPI.getStats(),
          bookingAPI.getAll(),
          vehicleAPI.getAll(),
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      toast.error(
        fr ? "Erreur chargement paiements" : "Error loading payments",
      );
    } finally {
      setPmtLoading(false);
    }
  ...
  const handleDeleteUser = async (userId: string) => {
    if (
      !window.confirm(fr ? "Supprimer cet utilisateur ?" : "Delete this user?")
    )
      return;
  ...
  const bookingBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 rounded-full text-xs font-semibold ${cls[status] ?? cls.PENDING}`}
      >
  ...
  const vehicleBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 rounded-full text-xs font-semibold ${cls[status] ?? cls.UNAVAILABLE}`}
      >
        {lbl[status] ?? status}
      </span>
    );
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            {
              label: t("admin.stats.fleet"),
              val: stats?.totalVehicles || 0,
              sub: `${stats?.availableVehicles || 0} ${t("admin.stats.available")}`,
              icon: (
                <FaCar className="text-4xl text-primary-light opacity-60" />
              ),
            ...
            },
            {
              ...
              sub: "",
              icon: (
                <FaCalendar className="text-4xl text-blue-400 opacity-60" />
              ),
              color: "text-primary-dark",
            },
            {
              label: t("admin.stats.pending"),
              val: stats?.pendingBookings || 0,
              sub: "",
              icon: <FaClock className="text-4xl text-yellow-400 opacity-60" />,
              color: "text-yellow-600",
            },
            {
              label: t("admin.stats.revenue"),
              val: `${Number(stats?.totalRevenue || 0).toLocaleString()} FCFA`,
              sub: "",
              icon: (
                <FaMoneyBill className="text-4xl text-green-400 opacity-60" />
              ),
        ...
        <div className="bg-white rounded-xl shadow-sm">
          {/* Tab bar */}
          <div className="border-b flex flex-col sm:flex-row sm:items-center sm:justify-between sm:pr-4">
            <div className="flex overflow-x-auto scrollbar-hide">
              {(
                [
                  {
                    key: "bookings",
                    label: `${t("admin.bookings")} (${bookings.length})`,
                    icon: <FaCalendar />,
                  },
                  {
                    key: "vehicles",
                    label: `${t("admin.vehicles")} (${vehicles.length})`,
                    icon: <FaCar />,
                  },
                  {
                    key: "users",
                  ...
                  {
                    key: "payments",
                    label: `${fr ? "Paiements" : "Payments"} (${payments.length})`,
                    icon: <FaReceipt />,
                  },
                ] as { key: TabType; label: string; icon: any }[]
              ).map((tab) => (
                <button
                  key={tab.key}
                  ...
                  {bookings.length === 0 ? (
                    ...
                    bookings.map((b) => (
                      <tr key={b.id} className="hover:bg-gray-50">
                        ...
                        <td
                          className={`${td} text-sm font-bold text-primary-dark`}
                        >
              ...
              <div className="p-4 border-b bg-gray-50 flex flex-wrap gap-3 items-end">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPmtFilters((p) => ({ ...p, dateFrom: e.target.value }))
                    }
                  />
                </div>
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPmtFilters((p) => ({ ...p, dateTo: e.target.value }))
                    }
                  />
                </div>
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setPmtFilters((p) => ({ ...p, status: e.target.value }))
                    }
                  >
              ...
              {!pmtLoading && payments.length > 0 && (
                <div className="px-4 py-2 bg-primary-dark/5 border-b flex gap-6 text-xs text-gray-600">
                  <span>
                    ...
                    {fr ? "paiement(s)" : "payment(s)"}
                  </span>
                  <span>
                    {fr ? "Total encaissé :" : "Total:"}{" "}
                    ...
                    {pmtLoading ? (
                      ...
                      payments.map((p: any) => {
                        ...
                        return (
                          ...
                          >
                            <td className={td}>
                              <p className="font-mono text-xs font-bold text-primary-dark">
                                {p.transactionId ?? "—"}
                              </p>
                              <p className="text-xs text-gray-400">
                            ...
                            <td
                              className={`${td} font-bold text-primary-dark text-sm`}
                            >
                              {Number(p.amount).toLocaleString()} {p.currency}
                            </td>
                            <td className={`${td} text-xs text-gray-600`}>
                              {mLbl[p.paymentMethod] ?? p.paymentMethod}
                            </td>
                            <td className={td}>
                              <span
                                className={`px-2.5 py-1 rounded-full text-xs font-semibold ${stCls[p.status] ?? "bg-gray-100 text-gray-600"}`}
                              >

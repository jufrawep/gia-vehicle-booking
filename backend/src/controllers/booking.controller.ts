...
export const createBooking = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    // FIXED: Use properly imported BookingStatus enum with explicit values
    ...
    const totalAmount = Number(vehicle.price_per_day) * days;
...
export const adminCreateBooking = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    // Check for date conflicts - Use properly imported enum
    ...
    if (conflicts.length > 0) {
      return next(new AppError('Vehicle already booked for these dates', 409));
    }

    const days = Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
    const totalPrice = days * Number(vehicle.price_per_day);
